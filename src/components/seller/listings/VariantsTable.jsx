...
const VariantsTable = ({ tiers, models, onChange, disabled, isEditMode = false }) => {
  ...
  const getVariantLabel = (model) =>
    model.tierIndex
      .map((idx, tierIdx) => tiers[tierIdx]?.options[idx]?.value || '?')
      .join(' / ');

  const getTierName = (tierIdx) => tiers[tierIdx]?.name || `Tier ${tierIdx + 1}`;

  const stockClass = (qty) => {
    if (!qty || qty === 0) return styles.stockEmpty;
    if (qty < 5) return styles.stockLow;
  ...
  const handleSelectAll = () => {
    setSelectedRows(selectedRows.size === models.length ? new Set() : new Set(models.map((_, i) => i)));
  };
  ...
  if (models.length === 0) {
    return (
      <div className={styles.emptyState}>
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" strokeWidth="1.5" style={{marginBottom:8}}>
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.tableHeader}>
        ...
        {selectedRows.size > 0 && (
          <div className={styles.bulkBar}>
            ...
            <input
              type="number" className={styles.bulkInput} placeholder="Price (₫)"
              value={bulkEdit.price}
              onChange={(e) => setBulkEdit({ ...bulkEdit, price: e.target.value })}
            />
            {!isEditMode && (
              <input
                type="number" className={styles.bulkInput} placeholder="Cost (₫)"
            ...
            {!isEditMode && (
              <input
                type="number" className={styles.bulkInput} placeholder="Stock"
                style={{ width: 80 }}
          ...
          <table className={styles.grid}>
            ...
            <tbody>
              {models.map((model, index) => (
                <tr
                  key={index}
                  className={selectedRows.has(index) ? styles.rowSelected : ''}
                >
                  {/* Checkbox */}
                  ...
                  <td className={styles.imgCell}>
                    {model.imagePreview || model.image ? (
                      <div className={styles.imgThumb}>
                        ...
                        />
                        <button
                          type="button" className={styles.imgRemoveBtn}
                      ...
                      <label className={`${styles.imgUploadZone} ${disabled ? styles.imgUploadDisabled : ''}`}>
                        <input
                          type="file" accept="image/*" className={styles.fileHidden}
                  ...
                  <td>
                    <input
                      type="number" className={styles.compactInput}
                      value={model.price || ''}
                  ...
                  <td style={{textAlign:'center'}}>
                    {isEditMode ? (
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px' }}>
                        ...
                        {model.costSource === 'po' && model.costSourcePoId ? (
                          <a
                            ...
                            style={{ fontSize: '0.62rem', color: '#2563eb', textDecoration: 'none', fontWeight: 600 }}
                          >
                            via PO ↗
                          </a>
                        ) : (
                      ...
                      <input
                        ...
                        onChange={(e) => handleModelChange(index, 'costPrice', parseFloat(e.target.value))}
                  ...
                  <td>
                    <input
                      type="text" className={styles.compactInput}
                      value={model.sku || ''}
      ...
      <div className={styles.tipRow}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2" className={styles.tipIcon}>
        ...
        <span>
          ...
          {isEditMode && <> Stock and cost price are managed via the <strong>Inventory</strong> page.</>}
        </span>
      </div>
    </div>
  );
