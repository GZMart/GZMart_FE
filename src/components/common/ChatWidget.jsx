...
const ChatWidget = () => {
    ...
    const fetchMessages = async (conversationId) => {
        try {
            ...
            setPage(1);
            setHasMore(true);
            const response = await axios.get(`${API_URL}/api/chat/messages/${conversationId}?page=1&limit=20`, {
                headers: { Authorization: `Bearer ${getAuthToken()}` }
            });
    ...
    const handleSendMessage = async (e) => {
        ...
        const receiverId = activeConversation.participants.find(p => (p._id || p) !== user._id)?._id ||
                    ...
                    {!isMinimized && (
                        <div className={styles['chat-widget-content']}>
                            ...
                            <div className={styles['chat-main']}>
                                ...
                                {(() => {
                                    const pinnedProduct = activeConversation && activeChatId !== 'ai'
                                    ...
                                    return (
                                        <div className={styles['pinned-product-banner']}>
                                            <img src={pinnedProduct.image} alt={pinnedProduct.name} className={styles['pinned-product-image']} />
                                            <div className={styles['pinned-product-info']}>
                                                <div className={styles['pinned-product-name']} title={pinnedProduct.name}>{pinnedProduct.name}</div>
                                                <div className={styles['pinned-product-tags']}>
                                                    <span className={styles['tag-orange']}><i className="bi bi-chat-text-fill"></i> Đã hỏi</span>
                                                    <span className="text-primary fw-medium" style={{ fontSize: '0.6rem' }}>Sản phẩm Shop</span>
                                                </div>
                                                <div className={styles['pinned-product-bottom']}>
                                                    <div className={styles['pinned-product-price-row']}>
                                                        <span className={styles['pinned-product-price']}>{formatCurrency(pinnedProduct.price)}</span>
                                                        {pinnedProduct.originalPrice && pinnedProduct.originalPrice > pinnedProduct.price && (
                                                    ...
                                                    {messages.map((msg, idx) => {
                                                        ...
                                                        return (
                                                            <div key={idx} className={`d-flex mb-3 ${isMe ? 'justify-content-end' : 'justify-content-start'}`}>
                                                                ...
                                                                <div className={`${styles['message-bubble']} ${isMe ? styles['message-user'] : styles['message-other']} ${msg.type === 'product' && msg.productInfo ? styles['message-bubble-product'] : ''}`}>
                                                                    {msg.type === 'product' && msg.productInfo ? (
                                                                        <ProductMessageCard productInfo={msg.productInfo} />
                                                                    ) : (
                                                                        <div>{msg.content}</div>
                                                                    )}
                                                                    <div className={`text-end mt-1 ${isMe && msg.type !== 'product' ? 'text-white-50' : 'text-muted'}`} style={{ fontSize: '0.65rem' }}>
                                ...
                                {activeConversation && activeChatId !== 'ai' && pendingProducts[activeConversation._id] && (
                                    <div className={styles['pending-product-wrapper']}>
                                        <div className={styles['pending-product-header']}>
                                            <span className={styles['pending-product-title']}>Bạn đang trao đổi với Người bán về sản phẩm này</span>
                                            <button
                                                type="button"
                                        ...
                                        <div className={styles['pending-product-card']}>
                                            <img src={pendingProducts[activeConversation._id].image} alt={pendingProducts[activeConversation._id].name} className={styles['pending-product-image']} />
                                            <div className={styles['pending-product-info']}>
                                                <div className={styles['pending-product-name']}>{pendingProducts[activeConversation._id].name}</div>
                                                <div className={styles['pending-product-price-row']}>
                                                    <span className={styles['pending-product-price']}>{formatCurrency(pendingProducts[activeConversation._id].price)}</span>
                                                    {pendingProducts[activeConversation._id].originalPrice && pendingProducts[activeConversation._id].originalPrice > pendingProducts[activeConversation._id].price && (
                                                        <span className={styles['pending-product-original-price']}>
                                                            {formatCurrency(pendingProducts[activeConversation._id].originalPrice)}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
