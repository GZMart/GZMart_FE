...
const BuyerReturnStatusDrawer = ({ open, requestId, onClose }) => {
  ...
  // Reset state when drawer opens with new requestId
  useEffect(() => {
    if (open && requestId) {
      setReturnRequest(null);
      setLegOneCompleted(false);
      setLegTwoCompleted(false);
      setExchangeLegOneCompleted(false);
      setExchangeLegTwoCompleted(false);
      fetchReturnRequest();
    }
  }, [open, requestId, fetchReturnRequest]);
  ...
  }, [open, requestId, user?._id, fetchReturnRequest]);
  ...
  // Derived values from returnRequest
  const resolutionType = returnRequest?.type;
  const logistics = returnRequest?.logistics || {};
  const logisticsSteps = Array.isArray(logistics.steps) ? logistics.steps : [];
    ...
    return (
      <>
        ...
        {isExchangeFlow && (
          <>
            <Alert variant="warning">
              <Alert.Heading style={{ fontSize: 15 }}>Exchange Logistics Flow</Alert.Heading>
              <ol className="mb-0" style={{ fontSize: 13 }}>
            ...
            <Card className="mt-3">
              <Card.Body>
                <h6 className="mb-3">Exchange Tracking Status</h6>
                  ...
                  <>
                    ...
                    <div className="mt-3 d-flex justify-content-end">
                      ...
                      >
                        {submitting
                          ? 'Submitting...'
                          : 'I Received Replacement & Handed Faulty Item'}
                      </Button>
                    </div>
                  </>
                )}
        ...
        {isRefundFlow && (
          <Card className="mt-3">
            <Card.Body>
              <h6 className="mb-3">Refund Tracking Status</h6>
              ...
              {(currentStep === 'buyer_to_seller_in_transit' ||
                ...
                <>
                  ...
                  {legTwoCompleted &&
                    ...
                    returnRequest.status !== 'completed' && (
                      <Alert variant="info" className="mt-3 mb-0">
                        Shipper has arrived at seller location. Waiting for seller to confirm
                        receipt.
                      </Alert>
                    )}
                </>
...
BuyerReturnStatusDrawer.propTypes = {
  open: PropTypes.bool.isRequired,
  requestId: PropTypes.string,
  onClose: PropTypes.func.isRequired,
};

export default BuyerReturnStatusDrawer;
