  ...
  return (
    <div className={styles.stepPanel}>
      <div className={styles.stepHint}>
        ...
        <span>
          Prepare your banner image. Recommended size: <strong>1200×600px</strong> — matches the
          GZMart homepage hero banner.
        </span>
      </div>
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}>
                  Supports: JPG, PNG, WebP · Ideal ratio 1200×600
                </div>
              </div>
            </div>
          </Dragger>
  ...
  return (
    <div className={styles.stepPanel}>
      <div className={styles.stepHint}>
        ...
        <span>
          Choose how the banner redirects users when clicked. You can either set a{' '}
          <strong>Default Link</strong> for the entire banner, or draw multiple{' '}
          <strong>Click Zones (Hotspots)</strong>.
        </span>
      </div>
          ...
          <div className={styles.hotspotSection}>
            <div className={styles.hotspotHeader}>
              <div>
                ...
                <div className={styles.hotspotSub}>
                  Draw click zones on the image to link to products or categories
                </div>
              </div>
              <Button
                icon={<AimOutlined />}
            ...
            {!imageUrl ? (
              <Alert
                type="warning"
                showIcon
                message="Please upload an image in Step 1 before adding hotspots"
              />
            ) : hotspots.length === 0 ? (
              <div
                style={{ textAlign: 'center', padding: '1.5rem', color: '#94a3b8', fontSize: 13 }}
              >
                  ...
                  <div key={i} className={styles.hotspotItem}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11, color: '#94a3b8', fontFamily: 'monospace' }}>
                        X: {Math.round(h.x)}% · Y: {Math.round(h.y)}% · W: {Math.round(h.width)}% ·
                        H: {Math.round(h.height)}%
                      </div>
                      <div
                        style={{
                          fontSize: 12,
                          color: h.link ? '#1677ff' : '#ef4444',
                          marginTop: 2,
                          overflow: 'hidden',
      ...
      <div className={styles.stepActions}>
        <Button onClick={onBack} icon={<ArrowLeftOutlined />} className={styles.backBtn}>
          Back
        </Button>
        <Button
          type="primary"
          ...
          className={styles.publishBtn}
          onClick={onNext}
          disabled={linkMode === 'hotspot' && hotspots.length === 0 && !imageUrl}
        >
          Next →
        </Button>
      </div>
  ...
  return (
    <div className={styles.stepPanel}>
      <Alert
        ...
        message={
          <span>
            Admin banners <strong>skip approval</strong> — they appear on the homepage immediately
            after publishing.
          </span>
        }
      />
      ...
      <div className={styles.configSection}>
        <div className={styles.configSectionTitle}>Banner Title</div>
        <Input
          value={bannerTitle}
      ...
      <div className={styles.configSection}>
        ...
        <div style={{ marginTop: 12 }}>
          <RangePicker
            ...
            onChange={(dates) => _setDateRange(dates || [null, null])}
            format="DD/MM/YYYY"
            minDate={dayjs()}
          />
        </div>
      ...
      <div className={styles.configSection}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ fontSize: 13, color: '#64748b', lineHeight: 1.5 }}>
            Lower number → appears <strong>first</strong> in the slideshow.
            <br />
        ...
        {adminBanners.length > 0 && (
          <div className={styles.orderPreviewList}>
            ...
            {[...adminBanners]
              ...
              .map((b, i) => (
                <div key={b._id} className={styles.orderPreviewItem}>
                  ...
                  <span style={{ flexShrink: 0, fontSize: 11, color: '#94a3b8', marginRight: 4 }}>
                    {b.ownerType === 'ADMIN' ? '🔵' : '🟠'}
                  </span>
                  <span
                    style={{
            ...
            <div
              className={styles.orderPreviewItem}
              style={{ border: '2px dashed #1677ff', background: '#eff6ff' }}
            >
              <div className={styles.orderPreviewNum} style={{ background: '#1677ff' }}>
                {parseInt(order) || 0}
              </div>
              <span style={{ fontSize: 11, color: '#1677ff', fontWeight: 700 }}>
                ⭐ Your new banner will be here
              </span>
            </div>
          </div>
        )}
...
const AdminBannerAdsPage = () => {
  ...
  const handleToggleStatus = async (banner) => {
    try {
      ...
      await bannerAdsService.adminUpdate(banner._id, {
        status: newStatus,
          ...
          <div className={styles.orderCell}>
            ...
            <div className={styles.orderBtns}>
              ...
              <Button
                ...
                disabled={idx === sortedAllBanners.length - 1}
                loading={reordering}
      ...
      render: (_, rec) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div className={styles.bannerInfo}>
            ...
            {rec.subtitle && (
              <div style={{ fontSize: 11, color: '#64748b', marginTop: 1 }}>{rec.subtitle}</div>
            )}
      ...
      render: (_, rec) => (
        <div style={{ fontSize: 12 }}>
          <div style={{ color: '#475569', marginBottom: 2 }}>
            👁 {(rec.metrics?.views || 0).toLocaleString()}
          </div>
          <div style={{ color: '#475569', marginBottom: 4 }}>
            🖱 {(rec.metrics?.clicks || 0).toLocaleString()}
          </div>
          <span
            style={{
              background: '#eff6ff',
          ...
          {rec.ownerType === 'ADMIN' ? (
            <>
              <Tooltip title={rec.status === 'RUNNING' ? 'Pause banner' : 'Resume banner'}>
                <Button
                  size="small"
                  icon={rec.status === 'RUNNING' ? <PauseCircleOutlined /> : <PlayCircleOutlined />}
                  onClick={() => handleToggleStatus(rec)}
                  style={{
                    borderRadius: 6,
                    color: rec.status === 'RUNNING' ? '#f59e0b' : '#10b981',
                    borderColor: rec.status === 'RUNNING' ? '#f59e0b' : '#10b981',
                  }}
                />
              </Tooltip>
            ...
            <Button
              ...
              onClick={() =>
                Modal.confirm({
                  ...
                  content:
                    rec.ownerType === 'SELLER'
                      ? 'Coins will be refunded to the Seller.'
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageInner}>
        ...
        <div className={styles.pageHeader}>
          <div className={styles.pageHeaderLeft}>
            {view === 'create' ? (
              <button onClick={resetWizard} className={styles.backIconBtn} title="Back to list">
                <ArrowLeftOutlined />
              </button>
            ) : (
            ...
            <div className={styles.listCardHeader}>
              <div>
                ...
                <div className={styles.listCardSub}>
                  {total} banners total · Sorted by display order
                </div>
              </div>
      ...
      <Modal
        ...
        okText="Confirm Reject & Refund Coins"
      ...
      >
        <Form form={rejectForm} layout="vertical" style={{ marginTop: 8 }}>
          <Form.Item
            ...
            rules={[
              { required: true, min: 5, message: 'Please enter a reason (min 5 characters)' },
            ]}
          >
            <Input.TextArea
              rows={4}
              placeholder="e.g. Image violates policy, low quality, misleading content..."
            />
          </Form.Item>
        </Form>
        <Alert
          ...
          <div>
                ...
                value: (
                  <span
                    style={{
                      padding: '2px 10px',
                      borderRadius: 20,
                      background: previewBanner.ownerType === 'ADMIN' ? '#eff6ff' : '#fff7ed',
                      color: previewBanner.ownerType === 'ADMIN' ? '#1677ff' : '#f97316',
                      fontWeight: 700,
                      fontSize: 12,
                    }}
                  >
                    {previewBanner.ownerType === 'ADMIN' ? '🔵 System' : '🟠 Seller'}
                  </span>
                ),
              },
              {
                ...
                value: (
                  <span
                    style={{
                      ...
                      fontSize: 12,
                      fontWeight: 600,
                      background: STATUS_CFG[previewBanner.status]?.bg,
                      color: STATUS_CFG[previewBanner.status]?.text,
                    }}
                  >
            ...
            {previewBanner.rejectionReason && (
              <Alert
                ...
                message={
                  <span>
                    <strong>Rejection Reason:</strong> {previewBanner.rejectionReason}
                  </span>
                }
              />
            )}
