...
const VOUCHER_TYPES = [
  {
    id: 'shop',
    title: 'Shop Voucher',
    description: 'Voucher applicable to all products in your Shop',
    icon: 'bi-shop',
  },
  {
    id: 'product',
    title: 'Product Voucher',
    description: 'Voucher only applicable to specific products selected by Shop',
    icon: 'bi-box-seam',
  },
  {
    id: 'private',
    title: 'Private Voucher',
    description: 'Voucher applicable to target customer groups via Voucher code',
    icon: 'bi-person-lock',
  },
  {
    id: 'live',
    title: 'Live Voucher',
    description: 'Exclusive voucher for products in livestream to boost conversion',
    icon: 'bi-broadcast',
  },
  {
    id: 'new_buyer',
  ...
  {
    id: 'repeat_buyer',
    title: 'Repeat Customer Voucher',
    description: 'Encourage existing customers to buy again from your shop',
    icon: 'bi-arrow-repeat',
  },
  {
    id: 'follower',
    title: 'Follower Offer',
    description: 'Encourage users to follow your Shop by rewarding them',
    icon: 'bi-heart',
  },
];
...
const VoucherDashboard = () => {
  ...
  const getStatusBadge = (startTime, endTime, status) => {
    ...
    if (isExpired) {
      return (
        <div
          className={`${styles.statusBadge} bg-secondary-subtle text-secondary border border-secondary-subtle`}
        >
          Expired
        </div>
      );
      ...
      return (
        <div
          className={`${styles.statusBadge} bg-warning-subtle text-warning border border-warning-subtle`}
        >
      ...
      return (
        <div
          className={`${styles.statusBadge} bg-success-subtle text-success border border-success-subtle`}
        >
          ...
          <div className={styles.bannerBackground}>
            ...
            <Card.Body className="d-flex align-items-center justify-content-between p-4 px-5">
              <div className="d-flex align-items-center position-relative" style={{ zIndex: 1 }}>
                ...
                <div>
                  ...
                  <p className="mb-0 text-secondary">
                    Potential to increase orders by{' '}
                    <span className="fw-bold text-success bg-success-subtle px-1 rounded">43%</span>{' '}
                    and revenue by{' '}
                    <span className="fw-bold text-success bg-success-subtle px-1 rounded">28%</span>
                    .
                  </p>
                </div>
              </div>
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button
                  className={`${styles.btnPrimary} shadow-sm px-4 py-2`}
                  size="lg"
                  onClick={() => navigate('/seller/vouchers/create/shop')}
                >
          ...
          <Row className="g-4">
            {VOUCHER_TYPES.map((type) => (
              <Col
                ...
                xl={20}
                key={type.id}
                style={{ flex: '1 0 20%', minWidth: '250px' }}
              >
                <motion.div whileHover={{ y: -5 }} className="h-100">
                  <div
                    className={styles.voucherCard}
                    onClick={() => navigate(`/seller/vouchers/create/${type.id}`)}
                  >
                      ...
                      {loading ? (
                        ...
                        sortedVouchers.map((voucher) => (
                          <tr key={voucher._id}>
                            ...
                            <td>
                              <span className={styles.textSecondary}>{voucher.type}</span>
                            </td>
                            <td>
                              ...
                              <div className="small text-muted">
                                Min. Spend ₫{voucher.minBasketPrice.toLocaleString()}
                              </div>
                            </td>
                            <td>
                            ...
                            <td>
                              {getStatusBadge(voucher.startTime, voucher.endTime, voucher.status)}
                            </td>
                            ...
                            <td className="text-end pe-4">
                              ...
                              >
                                <button
                                  className={`${styles.actionBtn} me-1`}
                                  onClick={() => navigate(`/seller/vouchers/view/${voucher._id}`)}
                                >
                              ...
                              {new Date(voucher.endTime) > new Date() && (
                                <OverlayTrigger
                                  placement="top"
                                  overlay={<Tooltip>Edit Voucher</Tooltip>}
                                >
                                  <button
                                    className={`${styles.actionBtn} me-1`}
                                    onClick={() => navigate(`/seller/vouchers/edit/${voucher._id}`)}
                                  >
