...
const VoucherCreatePage = () => {
  ...
  useEffect(() => {
    if (!isEdit && !isView) {
      // Only set defaults if creating new
      if (type === 'product') {
        setApplyTo('specific');
  ...
  const handleProductSelect = (products) => {
    ...
    const uniqueProducts = Array.from(
      new Map(products.map((item) => [item._id || item.id, item])).values()
    );
    ...
    <PageTransition>
      <Container fluid className={`p-4 ${styles.pageContainer}`}>
        ...
        <div className="d-flex align-items-center mb-4">
          ...
          <div>
            <h4 className="fw-bold mb-0 text-dark">
              {isEdit ? 'Edit' : isView ? 'View Details:' : 'Create'}{' '}
              {voucherType === 'product'
                ? 'Product'
                : voucherType === 'private'
                  ? 'Private'
            ...
            <div className="text-muted small mt-1">
              {isView ? 'View voucher configuration' : 'Define your voucher details to boost sales'}
            </div>
          </div>
        </div>
        ...
        <Row>
          ...
          <Col lg={4}>
            <VoucherPreview voucherType={voucherType} formData={formData} />
          </Col>
        </Row>
      </Container>
    </PageTransition>
