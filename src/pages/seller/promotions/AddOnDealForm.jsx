...
const AddOnDealForm = () => {
        ...
        try {
            ...
            if (data) {
                ...
                if (data.mainProducts) {
                    setMainProducts(data.mainProducts.map(p => ({
                        _id: p._id,
                        name: p.name,
                        images: p.images,
                ...
                if (data.subProducts) {
                    setSubProducts(data.subProducts.map(sp => {
    ...
    const getVariantName = (product, model) => {
        ...
        return model.tierIndex.map((idx, i) => product.tiers[i]?.options[idx]).filter(Boolean).join(' - ');
    };
    ...
    const updateSubProduct = (uniqueId, field, value) => {
        ...
        setSubProducts(prev => prev.map(p =>
            p._id === uniqueId ? { ...p, [field]: value } : p
        ));
    };
        ...
        <Card className={`${styles.section} ${activeStep < 1 ? styles.lockedSection : ''}`}>
            ...
            {activeStep === 1 && (
                <div className={styles.sectionContent}>
                    ...
                    <Form.Group className="mb-3">
                        <Form.Label className={styles.label}>Purchase Limit (Add-on items per order)</Form.Label>
            ...
            {activeStep === 2 && (
                <div className={styles.sectionContent}>
                    <div className="d-flex justify-content-between mb-3">
                        ...
                        {!isViewMode && (
                            <Button size="sm" onClick={() => setShowMainModal(true)} className={styles.addProductBtn}>
                                    ...
                                    <tr key={product._id}>
                                        ...
                                        {!isViewMode && (
                                            <td className="text-end">
                                                <Button variant="link" className="text-danger p-0 border-0" onClick={() => removeMainProduct(product._id)}>
            ...
            {activeStep === 3 && (
                <div className={styles.sectionContent}>
                    <div className="d-flex justify-content-between mb-3">
                        ...
                        {!isViewMode && (
                            <Button size="sm" onClick={() => setShowSubModal(true)} className={styles.addProductBtn}>
                                <i className="bi bi-plus-lg me-1"></i> Add Products
                            </Button>
                        )}
                    </div>
                            ...
                            <tbody>
                                {subProducts.map(product => (
                                    <tr key={product._id}>
                                        ...
                                        {!isViewMode && (
                                            <td className="text-end">
                                                <Button variant="link" className="text-danger p-0 border-0" onClick={() => removeSubProduct(product._id)}>
                                                    <i className="bi bi-trash"></i>
                                                </Button>
                                            </td>
                                        )}
                ...
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h4 className="fw-bold mb-1">
                            {isEditMode ? 'Edit Add-on Deal' : isViewMode ? 'View Add-on Deal' : 'Create Add-on Deal'}
                        </h4>
