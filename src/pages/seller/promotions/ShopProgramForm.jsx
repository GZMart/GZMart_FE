...
import { Container, Card, Row, Col, Form, Button, InputGroup, Table, OverlayTrigger, Tooltip, Spinner } from 'react-bootstrap';
...
const ShopProgramForm = () => {
                    ...
                    const mappedProducts = data.products.map(p => {
                        ...
                        return {
                            ...
                            variants: variants.map(v => {
                                ...
                                const currentStock = model.stock !== undefined ? model.stock : (product.totalStock || 0);
    ...
    const handleAddProducts = (selectedItems) => {
        ...
        setProducts(prev => [...prev, ...newProducts.filter(np => !prev.some(p => p._id === np._id))]);
    ...
    const toggleVariant = (productId, variantId) => {
        updateVariant(productId, variantId, 'enabled',
    ...
    const deleteSelectedProducts = () => {
        ...
        if (window.confirm(`Are you sure you want to delete ${selectedProductIds.length} selected product(s)?`)) {
            setVariantErrors(prev => {
                const next = { ...prev };
                products.forEach(p => {
    ...
    const validateForm = () => {
        ...
        const hasVariantErrors = Object.values(variantErrors).some(err => Object.keys(err).length > 0);
        if (hasVariantErrors) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await Promise.all(products.map(p =>
                shopProgramService.updateProductVariants(programId, p._id, p.variants.map(v => ({
                    variantId: v.id,
                    salePrice: v.salePrice,
                    discount: v.discount,
                    discountType: v.discountType,
            ...
            message.success(isEditMode ? 'Program updated successfully!' : 'Program created successfully!');
                        ...
                        <Form.Group as={Row} className="mb-3 align-items-center">
                            ...
                            <Col sm={3}>
                                <span className={styles.hint}>
                                    Program name will not be shown to buyers.
                                </span>
                            </Col>
                        </Form.Group>
                        ...
                        {showBatchSettings && products.length > 0 && (
                            <div className={styles.batchRow}>
                                <div className={styles.batchLabel}>
                                    ...
                                    />
                                    <div className="text-muted small">{selectedProductIds.length} product(s) selected</div>
                                </div>
                        ...
                        {products.length > 0 && (
                            <div className={styles.tableWrapper}>
                                <Table className={styles.productTable}>
                                    <thead>
                                        <tr>
                                            <th style={{ width: 40 }}><Form.Check type="checkbox" /></th>
                                            <th>Product Name</th>
                                                ...
                                                <OverlayTrigger
                                                    ...
                                                    overlay={
                                                        <Tooltip id="promo-qty-tooltip">
                                                            Number of products reserved for this Promotion Program. To ensure enough stock for the program, this quantity will be deducted from total stock until program ends. If set to &quot;No limit&quot;, all products will be sold at discount price throughout the program.
                                                        </Tooltip>
                                                    }
                                                >
                                                    <i className="bi bi-question-circle text-muted" style={{ fontSize: '12px', cursor: 'help' }}></i>
                                                </OverlayTrigger>
                                            </th>
                                            <th>
                                                Order Limit{' '}
                                                <OverlayTrigger
                                                    ...
                                                    overlay={
                                                        <Tooltip id="order-limit-tooltip">
                                                            Maximum number of products a customer can purchase in this Discount Program.
                                                        </Tooltip>
                                                    }
                                                >
                                                    <i className="bi bi-question-circle text-muted" style={{ fontSize: '12px', cursor: 'help' }}></i>
                                                            ...
                                                            <td>
                                                                <InputGroup size="sm" className={styles.priceInput} hasValidation>
                                                                    ...
                                                                    <Form.Control
                                                                        ...
                                                                        onChange={(e) => updateVariant(product._id, variant.id, 'salePrice', parseFloat(e.target.value))}
                                                            ...
                                                            <td>
                                                                <InputGroup size="sm" className={styles.discountInput} hasValidation>
                                                                    ...
                                                                    <Form.Control
                                                                        ...
                                                                        onChange={(e) => updateVariant(product._id, variant.id, 'discount', parseFloat(e.target.value))}
                                                            ...
                                                            <td>
                                                                <div className="d-flex align-items-center gap-1">
                                                                    <Form.Select
                                                                        ...
                                                                        onChange={(e) => updateVariant(product._id, variant.id, 'promoQtyType', e.target.value)}
                                                                        disabled={isViewMode}
                                                                    >
                                                                        <option value="unlimited">No limit</option>
                                                                        <option value="limited">Limited</option>
                                                                    ...
                                                                    {variant.promoQtyType === 'limited' && (
                                                                        <div className="position-relative">
                                                                            <Form.Control
                                                                                ...
                                                                                onChange={(e) => updateVariant(product._id, variant.id, 'promoQty', e.target.value)}
                                                                                placeholder="Qty"
                                                                                min="1"
                                                            ...
                                                            <td>
                                                                <div className="d-flex align-items-center gap-1">
                                                                    <Form.Select
                                                                        ...
                                                                        onChange={(e) => updateVariant(product._id, variant.id, 'orderLimitType', e.target.value)}
                                                                        disabled={isViewMode}
                                                                    >
                                                                        <option value="unlimited">No limit</option>
                                                                        <option value="limited">Limited</option>
                                                                    ...
                                                                    {variant.orderLimitType === 'limited' && (
                                                                        <div className="position-relative">
                                                                            <Form.Control
                                                                                ...
                                                                                onChange={(e) => updateVariant(product._id, variant.id, 'orderLimit', e.target.value)}
                                                                                placeholder="Limit"
                                                                                min="1"
