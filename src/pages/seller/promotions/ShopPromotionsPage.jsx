...
const MOCK_METRICS = {
  ...
  quantity: { value: 0, change: 0.0 },
  buyers: { value: 0, change: 0.0 },
};
...
const ShopPromotionsPage = () => {
  ...
  const fetchPrograms = async () => {
    ...
    try {
      ...
      const addonPrograms = (Array.isArray(addonData) ? addonData : []).map((p) => {
        const uniqueSubProducts = new Set(
          p.subProducts?.map((sp) => sp.productId?._id?.toString() || sp.productId?.toString())
        );
        return {
          _id: p._id,
          name: p.name,
      ...
      const allPrograms = [...shopPrograms, ...comboPrograms, ...addonPrograms].sort(
  ...
  const filteredPromotions = promotions.filter((p) => {
    ...
    if (start && end) {
      ...
      const isOverlapping =
        (promoStart.isBefore(filterEnd) || promoStart.isSame(filterEnd)) &&
    ...
    if (end < now) {
      ...
      return (
        <span className={`${styles.statusBadge} ${styles.statusUpcoming}`}>
          <i className="bi bi-clock-fill me-1" style={{ fontSize: '8px' }}></i>Upcoming
        </span>
      );
    } else {
      return (
        <span className={`${styles.statusBadge} ${styles.statusOngoing}`}>
          <i className="bi bi-circle-fill me-1" style={{ fontSize: '6px' }}></i>Ongoing
        </span>
      );
    }
  };
            ...
            <Col md={4} key={type.id}>
              <Card className={styles.typeCard}>
                <Card.Body className="d-flex align-items-start p-3">
                  ...
                  <Button className={styles.createBtn} onClick={() => navigate(type.link)}>
                    Create
                  </Button>
                </Card.Body>
              </Card>
            ...
            <Table hover className="mb-0" style={{ fontSize: '0.875rem' }}>
              <thead className={styles.tableHeader}>
                <tr>
                  ...
                  <th>
                    All <i className="bi bi-chevron-down ms-1" style={{ fontSize: '10px' }}></i>
                  </th>
                ...
                {loading ? (
                  ...
                  filteredPromotions.map((promo) => (
                    <tr key={promo._id}>
                      ...
                      <td>
                        <Link
                          to={`/seller/promotions/view/${promo.type === 'shop' ? 'shop' : promo.type === 'combo' ? 'combo' : 'addon'}/${promo._id}`}
                        ...
                        {new Date(promo.endTime) > new Date() && (
                          <Link
                            to={`/seller/promotions/edit/${promo.type === 'shop' ? 'shop' : promo.type === 'combo' ? 'combo' : 'addon'}/${promo._id}`}
                            className={`${styles.actionLink} ms-3`}
                          >
                            Edit
                          </Link>
                        )}
                        <Link to="#" className={`${styles.actionLink} ms-3`}>
                          Copy
                        </Link>
