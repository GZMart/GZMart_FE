...
import { Container, Card, Row, Col, Form, Button, Table, OverlayTrigger, Tooltip } from 'react-bootstrap';
...
const ComboPromotionForm = () => {
            ...
            if (data) {
                ...
                if (data.products) {
                    setProducts(data.products.map(p => ({
                        ...
                        stock: p.totalStock !== undefined ? p.totalStock : (p.models?.reduce((acc, m) => acc + (m.stock || 0), 0) || 0),
    ...
    const toggleProductEnabled = (productId) => {
        ...
        setProducts(prev => prev.map(p =>
            p._id === productId ? { ...p, enabled: !p.enabled } : p
        ));
    };
    ...
    const batchDisable = () => {
        ...
        setProducts(prev => prev.map(p =>
            selectedProductIds.includes(p._id) ? { ...p, enabled: false } : p
        ));
    };
    ...
    const batchEnable = () => {
        ...
        setProducts(prev => prev.map(p =>
            selectedProductIds.includes(p._id) ? { ...p, enabled: true } : p
        ));
    };
    ...
    const batchDelete = () => {
        ...
        if (window.confirm(`Are you sure you want to delete ${selectedProductIds.length} product(s)?`)) {
