...
const ProductGroup = ({ group, index, exchangeRate, onUpdate, onRemove }) => {
  ...
  const autoFillWeight = (vi) => {
    ...
    const newVariants = variants.map((v) => ({
      ...v, weightKg: src.weightKg, dimLength: src.dimLength,
      dimWidth: src.dimWidth, dimHeight: src.dimHeight,
    }));
    onUpdate({ ...group, variants: newVariants });
  };
  ...
  return (
    <div className={styles.productGroup}>
      ...
      <div className={styles.pgHeader}>
        ...
        <button type="button" className={styles.btnRemove} onClick={onRemove} title="Remove group">✕</button>
      </div>
                ...
                <tr key={vi}>
                  ...
                  <td>
                    <input type="number" min="1" className={styles.numInput}
                      value={v.quantity} onChange={(e) => updateVariant(vi, 'quantity', e.target.value)} />
                  </td>
                  <td>
                    <input type="number" min="0" step="0.5" className={styles.numInput}
                      value={v.unitPriceCny} onChange={(e) => updateVariant(vi, 'unitPriceCny', e.target.value)} />
                  </td>
                  <td className={styles.calcCell}>
                    {fmt(Number(v.unitPriceCny) * exchangeRate * Number(v.quantity))} ₫
                  </td>
                  <td>
                    <input type="number" min="0" step="0.01" className={styles.numInput} style={{ width: 65 }}
                    ...
                    <td>
                      <div className={styles.dimRow}>
                        <input type="number" min="0" step="0.5" className={styles.dimInput}
                          value={v.dimLength} onChange={(e) => updateVariant(vi, 'dimLength', e.target.value)} placeholder="L" />
                        <input type="number" min="0" step="0.5" className={styles.dimInput}
                          value={v.dimWidth} onChange={(e) => updateVariant(vi, 'dimWidth', e.target.value)} placeholder="W" />
                        <input type="number" min="0" step="0.5" className={styles.dimInput}
                          value={v.dimHeight} onChange={(e) => updateVariant(vi, 'dimHeight', e.target.value)} placeholder="H" />
                      </div>
                    </td>
                  )}
                  <td>
                    {vi === 0 && variants.length > 1 && (
                      <button type="button" className={styles.btnCopyDim}
                        onClick={() => autoFillWeight(vi)} title="Copy kg/dim to all variants">
                        ↓ Copy
                      </button>
                    )}
                  </td>
...
const EditPurchaseOrderPage = () => {
  ...
  useEffect(() => {
    ...
    if (po.items?.length) {
      ...
      po.items.forEach((item) => {
        ...
        if (!groupMap.has(key)) {
          groupMap.set(key, { _id: Math.random().toString(36).slice(2), productName: key, tiers: [], variants: [] });
        }
  ...
  const validate = () => {
    ...
    if (!expectedDeliveryDate) errs.expectedDeliveryDate = 'Please select a delivery date';
    groups.forEach((g, gi) => {
      if (!g.productName.trim()) errs[`g${gi}_name`] = 'Product name cannot be empty';
  ...
  return (
    <div className={styles.container}>
      ...
      <div style={editBannerStyle}>
        <WarnIcon />
        <span>
          You are <strong>editing</strong> order{' '}
          <strong>{po.code}</strong>. Click <strong>Save Changes</strong> at the bottom to save.
        </span>
      </div>
      ...
      <div className={styles.section}>
        <div className={styles.sectionHeader}>
          <h2 style={{ marginBottom: 0 }}>
            Products List
            <span style={{ fontSize: '0.75rem', fontWeight: 500, color: '#94a3b8', textTransform: 'none', letterSpacing: 0, marginLeft: 8 }}>
              ({groups.reduce((s, g) => s + g.variants.length, 0)} SKU · {groups.length} groups)
            </span>
          </h2>
          <button type="button" className={styles.btnSecondary} onClick={() => setGroups((p) => [...p, EMPTY_GROUP()])}
...
const errorBannerStyle = {
  ...
  fontSize: '0.875rem', color: '#991b1b', marginBottom: '1rem',
};
