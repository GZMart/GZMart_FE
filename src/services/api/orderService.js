...
export const orderService = {
  ...
  // Get current user's orders with pagination and optional status filter
  getMyOrders: async (page = 1, limit = 10, status = 'all') =>
    await api.get('/api/orders', { params: { page, limit, ...(status !== 'all' && { status }) } }),
